...
\begin{center}
  \begin{tikzpicture}[scale=.8]
    \draw[thick] (0,0)  -- (2,0)node[anchor=north west] {$D$} ;
    \draw[thick] (0,0) node[anchor=north west] {$A$} -- (0,2) node[anchor=north west]{$B$};
    \draw[thick] (0,0) -- (0,-2) node[anchor=north west]{$C$};
  \end{tikzpicture}
 \end{center}
$ \text{Circ}(A,B,C) \rightleftharpoons \bot(C,A,B) \lor \text{Eq}(A,B) \lor \text{Eq}(B,C). $\\
\begin{center}
  \begin{tikzpicture}[scale=.8]
    \draw[thick] (0,2) node[anchor=north west] {$A$}  -- (4,2)node[anchor=north west] {$B$} ;

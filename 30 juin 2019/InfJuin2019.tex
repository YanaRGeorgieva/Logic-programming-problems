...
\begin{tikzpicture}[->,,auto,node distance=1.75cm,
  ...
  \path
    ...
    (1) edge[blue] [bend left] node {} (3)
        edge[red] [bend right] node {} (2)
        edge[red] [bend right] node {} (5)
        edge[red] [bend left] node {} (6)                                          ребра от изходен връх node с $h^S(node)$ до върхове на следващи нива с четности спрямо $h^S: \,\{0,1,2\}\ \setminus \{h^S(node)\}$\\
        edge[red] [bend right] node {} (3)
        edge[red] [bend left] node {} (4)
    (2) edge[red] [bend right] node {} (5)
        edge[red] [bend left] node {} (3)
        edge[red] [bend right] node {} (4)
        edge[blue] [bend right] node {} (6)
        edge[red] [bend left] node {} (6);      
\end{tikzpicture} \\
$ S = (\mathbb{N}, p^S, q^S, h^S)$ \\
$h^S(node) = node\,(mod3), node\in \mathbb{N}$\\
$\textcolor{blue}{q^S} \rightleftharpoons \{(node_1, node_2)\,|\, node_1 \in \mathbb{N}\,\&\, node_2 \in \mathbb{N}\,\&\,$\\$ node_1 < node_2 \,\&\, node(mod 3) \neq node_2(mod 3) \}$\\                                          ребра от изходен връх node с $h^S(node)$ до върхове на следващи нива с четности спрямо $h^S: \,\{0,1,2\}\ \setminus \{h^S(node)\}$\\
$\textcolor{red}{p^S} \rightleftharpoons \{(node_1, node_2)\,|\, node_1 \in \mathbb{N}\,\&\, node_2 \in \mathbb{N}\,\&\, node_1 < node_2 \}$\\
\end{center}
\end{addmargin}
